...
export class AuthService {
  ...
  async login(email: string, password: string) {
    ...
    if (!plumber) throw new UnauthorizedException('Email ou mot de passe incorrect')
    ...
    const payload = { sub: plumber.id, email: plumber.email, role: plumber.role }
  ...
  async me(plumberId: number) {
    const plumber = await this.prisma.plumber.findUnique({
      ...
      select: {
        ...
        phone: true, verified: true, role: true, createdAt: true,
      },
    })
